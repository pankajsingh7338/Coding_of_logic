...
public class Hashids {
	...
	public Hashids(String salt, int minHashLength, String alphabet) {
		...
		alphabet_ = join(
				new LinkedHashSet<String>(Arrays.asList(alphabet.split(""))),
				"");
	...
	private String encode(long[] numbers, String alphabet, String salt,
			int minHashLength) {
		String ret = "";
		...
		for (int i = 0; i < numbers.length; i++) {
			if (i == 0) {
				...
				alphabet = lotteryChar
						+ alphabet.replaceAll(String.valueOf(lotteryChar), "");
			}

			alphabet = consistentShuffle(alphabet, ((int) lotteryChar & 12345)
					+ salt);
		...
		while (ret.length() < minHashLength) {
			long[] padArray = new long[] { alphabet.charAt(1),
					alphabet.charAt(0) };
	...
	private long[] decode(String hash) {
		...
		if (hash != null && !hash.isEmpty()) {
			...
			hash = hashSplit[hashSplit.length == 3 || hashSplit.length == 2 ? 1
					: 0];
			...
			for (int i = 0; i < hashArray.length; i++) {
				...
				if (subHash != null && !subHash.isEmpty()) {
					if (i == 0) {
						...
						alphabet = lotteryChar
								+ alphabet_.replaceAll(
										String.valueOf(lotteryChar), "");
					}
				}

				if (alphabet.length() > 0) {
					alphabet = consistentShuffle(alphabet,
							((int) lotteryChar & 12345) + salt_);
	...
	private static String consistentShuffle(String alphabet, String salt) {
		...
		if (!alphabet.isEmpty()) {
			List<String> alphabetArray = charArrayToStringList(alphabet
					.toCharArray());
