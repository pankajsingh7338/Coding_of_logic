...
public class Request {
	...
	public Request(InetSocketAddress socketAddress, FullHttpRequest request,
			RouteResolver routes, SerializationProvider serializationProvider) {
	...
	/**
	 * Perform content-type negotiation and return the best-match Media-Type as
	 * a string from this request.
	 * 
	 ...
	 * However, if the ByteBuf returns false to hasArray(), this method returns
	 * null.
	 * 
	 * @return an array of byte, or null, if the ByteBuf is not backed by a byte
	 *         array.
	 */
	...
	public Map<String, List<String>> getBodyFromUrlFormEncoded(
		...
		if (shouldDecode) {
			QueryStringDecoder qsd = new QueryStringDecoder(getBody().toString(
					ContentType.CHARSET), ContentType.CHARSET, false);
			return qsd.parameters();
		}

		QueryStringParser qsp = new QueryStringParser(getBody().toString(
	 ...
	 * @throws BadRequestException
	 *             if 'name' doesn't exist as a header.
	 */
	...
	public boolean isChunked() {
		// This is the logic the Netty 3.9.x used to determine if data was
		// chunked. There may be a methodology more
		// inline with Netty 4.x.x to determine if the request is chunked. TODO:
		// Implement updated logic.
		for (String header : httpRequest.headers().getAll(
	...
	public boolean isHeaderEqual(String name, String value) {
		...
		if (header == null || header.trim().length() == 0 || value == null
				|| value.trim().length() == 0)
			return false;
	...
	public boolean isHttpVersion1_0() {
		return ((httpVersion.majorVersion() == 1) && (httpVersion
				.minorVersion() == 0));
	}
	...
	private void parseQueryString(final HttpRequest request) {
		...
		Map<String, List<String>> parameters = new QueryStringParser(
				request.getUri(), true).getParameters();
		...
		for (Entry<String, List<String>> entry : parameters.entrySet()) {
			...
			for (String value : entry.getValue()) {
				try {
					request.headers().add(entry.getKey(),
							URLDecoder.decode(value, ContentType.ENCODING));
				} catch (Exception e) {
	...
	private void determineEffectiveHttpMethod(HttpRequest request) {
		...
		String methodString = request.headers().get(
				Parameters.Query.METHOD_TUNNEL);

		if ("PUT".equalsIgnoreCase(methodString)
				|| "DELETE".equalsIgnoreCase(methodString)) {
			effectiveHttpMethod = HttpMethod
					.valueOf(methodString.toUpperCase());
		}
	}

	private void createCorrelationId() {
		this.correlationId = new ObjectId().toString();
	}
	...
	public void setUaSource(String uaSource) {
		this.uaSource = uaSource;
	}
}
