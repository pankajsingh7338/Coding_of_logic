...
import org.restexpress.route.SecuirtyHandler;
...
public class RestExpress {
	 ...
	 *            a SerializationProvider instance.
	 */
	public static void setDefaultSerializationProvider(
	...
	public RestExpress setSecuirtyHandler(SecuirtyHandler secuirtyHandler) {
		this.secuirtyHandler = secuirtyHandler;
		return this;
	}

	/**
	 * Turns off the Netty HttpContentCompressor on binding so that output GZip
	 * and deflate encodings are not possible. This is a speed optimization, per
	 * Issue #126
	 * 
	...
	/**
	 * Answers whether the service is setup to use response compression via the
	 * Netty HttpContentCompressor.
	 * 
	 * @return true if the RestExpress server is configured to use response
	...
	public ChannelHandler buildRequestHandler() {
		...
		requestHandler.setSecuirtyHandler(secuirtyHandler);
	...
	public Channel bind(InetSocketAddress ipAddress) {
		ServerBootstrap bootstrap = bootstrapFactory
				.newServerBootstrap(getIoThreadCount());
		...
		if (shouldUseSystemOut()) {
			System.out.println(getName() + " server listening on port "
					+ ipAddress.toString());
		}
	...
	private RouteResolver createRouteResolver() {
		return new RouteResolver(
				routeDeclarations.createRouteMapping(routeDefaults));
	}
